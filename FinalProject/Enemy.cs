...
namespace FinalProject
{
    ...
    {
        public Enemy(bool isHorizontalMovement,double wayTimeLimit) : base(isHorizontalMovement,wayTimeLimit)
        {
        }
        public void LoadContent(ContentManager Content, Rectangle pos, double timeAnimation, Point speed)
        {
            ...
            ///kid
            AddState(Content, state.left.ToString(), new Point(-Math.Abs(speed.X), 0), timeAnimation, "Sprites/enemy/EnemyLeft",0, 3, 32, 32, pos, Color.White);
            AddState(Content, state.right.ToString(), new Point(Math.Abs(speed.X), 0), timeAnimation, "Sprites/enemy/EnemyRight", 0, 3, 32, 32, pos, Color.White);
            AddState(Content, state.up.ToString(), new Point(0, -Math.Abs(speed.Y)), timeAnimation, "Sprites/enemy/EnemyBack", 0, 3, 32, 32, pos, Color.White);
            AddState(Content, state.down.ToString(), new Point(0, Math.Abs(speed.Y)), timeAnimation, "Sprites/enemy/EnemyFront", 0, 3, 32, 32, pos, Color.White);
            AddState(Content, state.staticLeft.ToString(), new Point(0, 0), timeAnimation, "Sprites/enemy/EnemyLeft", 8, 8, 460, 600, pos, Color.White);
            AddState(Content, state.staticRight.ToString(), new Point(0, 0), timeAnimation, "Sprites/enemy/EnemyRight", 7, 7, 460, 600, pos, Color.White);
            AddState(Content, state.staticUp.ToString(), new Point(0, 0), timeAnimation, "Sprites/enemy/EnemyBack", 12, 12, 460, 600, pos, Color.White);
            AddState(Content, state.staticDown.ToString(), new Point(0, 0), timeAnimation, "Sprites/enemy/EnemyFront", 0, 0, 460, 600, pos, Color.White);
            ...
            base.LoadContent(Content, "Enemy");
            //this.speed = speed;
            color = Color.White;
            this.pos = pos;
